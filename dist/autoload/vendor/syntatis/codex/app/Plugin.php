...
namespace SSFV\Codex;

use SSFV\Codex\Abstracts\Facade;
use SSFV\Codex\Abstracts\ServiceProvider;
use SSFV\Codex\Contracts\Activatable;
use SSFV\Codex\Contracts\Bootable;
use SSFV\Codex\Contracts\Deactivatable;
use SSFV\Codex\Contracts\Extendable;
use SSFV\Codex\Contracts\Hookable;
use SSFV\Codex\Core\App;
use SSFV\Codex\Core\Config;
use SSFV\Codex\Foundation\Blocks;
use SSFV\Codex\Foundation\Hooks\Hook;
use InvalidArgumentException;
use SSFV\Pimple\Container as PimpleContainer;
use SSFV\Pimple\Psr11\Container;
...
final class Plugin
{
    ...
    public function boot() : void
    {
        ...
        if ($this->ext instanceof Activatable && is_file($this->pluginFilePath)) {
            $ext = $this->ext;
            register_activation_hook($this->pluginFilePath, fn() => $ext->activate($this->container));
        }
        if ($this->ext instanceof Deactivatable && is_file($this->pluginFilePath)) {
            $ext = $this->ext;
            register_deactivation_hook($this->pluginFilePath, fn() => $ext->deactivate($this->container));
        }
        /** @var App $app */
    ...
    private function registerCoreServices() : void
    {
        ...
        $this->pimple['config'] = static function (PimpleContainer $container) : Config {
            ...
            if (is_dir($configPath)) {
                ...
                foreach ($iterator as $configFile) {
                    if (!$configFile instanceof SplFileInfo || $configFile->getExtension() !== 'php') {
                        continue;
                    }
                    $config[$configFile->getBasename('.php')] = (include $configFile->getRealPath());
                }
            }
            if (!isset($config['app']['name']) || !is_string($config['app']['name']) || trim($config['app']['name']) === '') {
        ...
        $this->pimple['app'] = static function (PimpleContainer $container) : App {
            ...
            return new App($name, ['plugin_dir_path' => $pluginDirPath, 'plugin_file_path' => $pluginFilePath]);
        };
    }
}
