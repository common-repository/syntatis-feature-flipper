...
use Closure;
use SSFV\Codex\Foundation\Hooks\Exceptions\RefExistsException;
use SSFV\Codex\Foundation\Hooks\Support\Parser;
...
final class Hook
{
     ...
     */
    public function addAction(string $tag, callable $callback, int $priority = 10, int $acceptedArgs = 1, array $options = []) : void
    {
     ...
     * @param array<string, mixed> $options      Optional. Additional options for the action.
     */
    public function addFilter(string $tag, callable $callback, int $priority = 10, int $acceptedArgs = 1, array $options = []) : void
    {
    ...
    {
        $hooks[] = ['accepted_args' => $acceptedArgs, 'callback' => $callback, 'tag' => $tag, 'priority' => $priority];
        return $hooks;
    }
    ...
    private function addRef(string $id, string $nativeId, array $entry) : void
    {
        if ($nativeId !== $id) {
            ...
            $this->refs[$nativeId] = ['callback' => $entry['callback']];
        }
    }
    ...
    private function getNamedId(array $options = []) : ?string
    {
        if (isset($options['id']) && is_string($options['id']) && trim($options['id']) !== '') {
            ...
            if (count($matches) === 0) {
                throw new InvalidArgumentException('Invalid ref ID format. A ref ID should only contains letters, numbers, hyphens, dots, underscores, and backslashes.');
            }
            return $options['id'];
        }
        return null;
