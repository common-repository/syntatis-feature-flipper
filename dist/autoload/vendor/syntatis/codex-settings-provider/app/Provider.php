...
class Provider extends ServiceProvider implements Hookable
{
    public function register() : void
    {
        $this->container[Settings::class] = static function (Container $container) : Settings {
            ...
            $settingFiles = new RecursiveDirectoryIterator($settingsDir, RecursiveDirectoryIterator::SKIP_DOTS);
            ...
            foreach ($settingFiles as $settingFile) {
                if (!$settingFile instanceof SplFileInfo || !$settingFile->isFile() || $settingFile->getExtension() !== 'php') {
                    continue;
                }
    ...
    public function hook(Hook $hook) : void
    {
        ...
        foreach ($settingsService as $group => $registry) {
            ...
            foreach ($settings as $setting) {
                $hook->addFilter('default_option_' . $setting->getName(), static function ($default, $option, $passedDefault) use($setting) {
                    return $passedDefault ? $default : $setting->getDefault();
                }, 10, 3, ['id' => 'settings-provider/default_option_' . $setting->getName()]);
            }
            $hook->addAction('admin_init', [$registry, 'register'], 10, 1, ['id' => 'settings-provider/admin_init_' . $group]);
            $hook->addAction('rest_api_init', [$registry, 'register'], 10, 1, ['id' => 'settings-provider/rest_api_init' . $group]);
        }
    }
}
